...
float getFilteredShadowing(sampler2DShadow tex, vec4 coord, vec4 offsx, mat4 matrix)
{
    /*
    // failed experiment
    vec4 coordzq = coord;
    coordzq.xyz /= coordzq.w;
    vec2 svz = coordzq.xy * vec2(textureSize2D(tex, 0));
    float sz = texelFetch2D(tex, ivec2(svz), 0).r;
    
    vec4 coordzs = coord + offsx + matrix * vec4(0.0, 0.0, 1.0, 1.0);
    coordzs.xyz /= coordzs.w;
    vec2 svzs = coordzs.xy * vec2(textureSize2D(tex, 0));
    float szs = texelFetch2D(tex, ivec2(svzs), 0).r;
    
    float bf = (coordzq.z - sz) / (abs(coordzq.z - coordzs.z) + 0.00001);
    bf = clamp(bf, 0.0, 1.0);
    ...
    */
    ...
    int _FILTER_SIZE = ((PBR_SHADOW_FILTER_SIZE) < 1 ? 1 : (PBR_SHADOW_FILTER_SIZE) > 16 ? 16 : (PBR_SHADOW_FILTER_SIZE));
    
    float scale = 1.0 / (PBR_SHADOWMAP_RES) * coord.w;
    //float scale = 1.0 / (PBR_SHADOWMAP_RES);
...
// plane-dependent filtering
    ...
    float amount = 0.0;
    float norm = 0.0;
    float ends = (_FILTER_SIZE - 1.0) / 2.0;
    ...
    for (float y = -ends; y < ends + 0.1; y++)
    {
        for (float x = -ends; x < ends + 0.1; x++)
        {
            ...
            //innercoord.xyz /= innercoord.w;
            //if (innercoord.z > 1.0) continue;
            //vec2 sv = innercoord.xy * vec2(textureSize2D(tex, 0));
            //float s = texelFetch2D(tex, ivec2(sv), 0).r;
            //amount += innercoord.z < s ? 1.0 : 0.0;
